...
def load_radio_map():
    ap_num = 81; col = 16; row = 18
    radio_map = [[[] for _ in range(ap_num) for _ in range(col)] for _ in range(row)]
    # [18][16][81]의 radio map 미리 생성
    fp = open('rss_value_zero.txt', 'r')
    
    for line in fp.readlines():
        coord_line = line[0:323].rstrip().split(',') # 숫자로 잘라냄
...
def calculate_client_position(radio_map, client_rss_store_median, ap_list):
    ap_num = 81; row = 18; col = 16
